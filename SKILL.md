...
- **版本**: v6.0（模块化 — `app/` 多子模块）
- **认证**: Bearer Token。`POST /api/tokens` 创建 token，之后每个 `/api/` 请求携带 `Authorization: Bearer <token>`。免认证端点：`/health`、`/api/events`、`POST /api/tokens`、`/skill`、静态页。
...
- `project`：自由文本，用于按项目过滤，例如 `todo-web`、`narrative-engine`。为空归入"未分配"
- `tag` / `tags`：单标签字符串 / 标签数组，两种形式兼容
- `dueDate` / `startDate` / `completedAt`：ISO 日期字符串（`YYYY-MM-DD` 或 `YYYY-MM-DDTHH:MM:SSZ`）
- `notes`：数组，每项 `{author, text, createdAt}`

## Token 认证

```bash
# 创建 token（免认证）
curl -X POST http://localhost:5003/api/tokens \
  -H "Content-Type: application/json" \
  -d '{"owner": "wangcai"}'
# 响应：{"success":true,"token":{"token":"raw-token-...","id":1,...}}
...
# 或通过 query 参数
curl "http://localhost:5003/api/tasks?api_token=<raw-token>"

# 撤销 token
curl -X DELETE http://localhost:5003/api/tokens/{id} \
  -H "Authorization: Bearer <raw-token>"
```
...
`active` = 非 `completed` 且非 `cancelled` 的任务数。`project` 为空字符串表示未分配项目的任务汇总。
...
```
server.py          # 薄入口：日志初始化 + uvicorn 启动
app/
  config.py        # 常量 / 环境变量
  database.py      # psycopg2 连接池 + ensure_schema()（自动建表/补列）
  utils.py         # 工具函数
  response.py      # ok() 响应助手
  sse.py           # SSE 队列管理
  middleware.py    # Bearer Token 认证中间件 + 请求日志
  services/        # 纯业务逻辑（无 HTTP）
    tasks.py       # 任务 CRUD，支持 project 过滤
    tokens.py      # Token CRUD + verify
    stats.py       # 统计查询
    logs.py        # 操作日志
  routes/          # FastAPI 路由注册
    tasks.py  stats.py  logs.py  tokens.py  sse.py  projects.py
  main.py          # app 工厂，注册路由 + 中间件
```
